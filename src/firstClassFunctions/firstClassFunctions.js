...
function formatProducts(product) {
  ...
  function formatProducts(carousel, grid, sidebar) {
    const formatProductsArray = (products) => products.map(formatProductName);

    const carouselProductsReformatted = formatProductsArray(carouselProducts);
  const gridProductsReformatted = formatProductsArray(gridProducts);
  const sidebarProductsReformatted = formatProductsArray(sidebarProducts);
